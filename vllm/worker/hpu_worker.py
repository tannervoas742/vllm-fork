...
def init_worker_distributed_environment(
    ...
    if torch.distributed.is_initialized():
        ...
        print(f"\n\n Using Distributed Init Method {rank} \n\n")
        backend = hpu_backend_string()
    ...
    device = torch.device(hpu_device_string())
    #device = torch.device(f"hpu:{local_rank}")
    device = hpu_device_string()
    import socket
    hostname = socket.gethostname()
    IPAddr = socket.gethostbyname(hostname)
    print(f"\n\n My Device: {device} for rank {rank} and local rank {local_rank} with IP {IPAddr} and name {hostname} \n\n")
